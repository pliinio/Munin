...
def _scan_single_host(ip: str, profile: str, do_cve: bool, do_nse: bool,
                      log_entries: list | None = None) -> dict:
    host: dict = {
...
def run_full_network_scan(target: str, profile: str, do_cve: bool, do_nse: bool) -> dict:
    start = time.time()
    result: dict = {
        "meta": {
            "target":        target,
...
def run_single_host_scan(ip: str, profile: str, do_cve: bool, do_nse: bool) -> dict:
    start = time.time()
    console.print()
...
def cmd_readlog(path_str: str) -> None:
    ...
    # Style map for level/status cells
    ...
    # ── Threat correlation on log entries ────────────────────────────────────
...
def _run_log_correlation(entries: list, log_type: str) -> None:
    ...
    # Build a minimal host_data with only log context
    host_data = {
        "ip": "log-only",
        "ports": [],
        "vulnerabilities": {},
...
def _export_log_md(columns: list, entries: list, source_path: str) -> None:
    ...
    # Data rows — strip Rich markup for plain Markdown
    import re
...
def _ask(prompt: str) -> str:
    """Plain input with the given prompt string."""
    try:
        return input(prompt)
...
def dispatch(line: str, s: Settings, last_result: dict) -> dict:
    ...
    # ── scan host <IP> ───────────────────────────────────────────────────────
        ...
        if ans in ("y", "yes"):
            last_result = run_single_host_scan(ip, s.profile, s.do_cve, s.do_nse)
            save_and_report(last_result, s)
