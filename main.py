...
def _print_business_reports(result: dict, audience: str = "manager") -> None:
    ...
    console.print()
    console.rule(f"[cyan]Relatório de Negócios — audience: {audience}[/cyan]")
...
def _render_business_report_dict(report: dict) -> None:
    ...
    console.print(
        ...
        f"[dim]urgência: {urgency}  ·  gerado por: {gen_by}  ·  audience: {audience}[/dim]"
    )
    console.print()
    ...
    if impact:
        console.print(f"  [bold]Impacto nos Negócios[/bold]")
        console.print(f"  {impact}")
        console.print()

    flags = report.get("compliance_flags", [])
    if flags:
        console.print(f"  [bold]Indicadores de Conformidade[/bold]")
        for flag in flags:
...
def _save_business_report_md(result: dict, audience: str) -> Path:
    ...
    lines = [
        f"# Relatório de Negócios Munin — {audience.capitalize()}",
    ...
    # ── assets ── v2 ─────────────────────────────────────────────────────────
        ...
        if not last_result.get("hosts"):
            ...
            try:
                ...
                console.print()
                console.rule("[cyan]Classificação de Criticidade de Ativos[/cyan]")
                t = Table(box=box.SIMPLE_HEAD, border_style="dim", header_style="bold cyan", padding=(0,1))
                t.add_column("IP",              style="cyan", width=16)
                t.add_column("Tipo de Ativo",   width=22)
                t.add_column("Criticidade",     width=12)
                t.add_column("Confiança",       width=10)
                t.add_column("Pontuação",       width=12)
                t.add_column("Motivo",          style="dim", width=40)
                ...
                for h in sorted(hosts, key=lambda x: x.get("risk_score",0), reverse=True):
                    ...
                    reason = (ap.get("type_reason") or "heurística")[:38]
                    t.add_row(
                        h.get("ip","?"),
                        atype,
                        f"[{cc}]{crit}[/{cc}]",
    ...
    # ── compliance ─────────────────────────────────────────────────────────────
        ...
        if not last_result.get("hosts"):
            ...
            try:
                ...
                t.add_row("Exposição LGPD", f"[{lc}]{lgpd}[/{lc}]")
                console.print(t)
            except Exception as exc:
    ...
    # ── remediation ────────────────────────────────────────────────────────────
        ...
        if not last_result.get("hosts"):
            ...
            try:
                ...
                t = Table(box=box.SIMPLE_HEAD, border_style="dim", header_style="bold cyan", padding=(0,1))
                t.add_column("IP",          style="cyan", width=16)
                t.add_column("Prioridade",  width=12)
                t.add_column("Pontuação",   width=8)
                lc_map = {"Imediato":"bold red","Alto":"red","Médio":"yellow","Planejado":"green"}
                for plan in plans:
                ...
                console.print(t)
                console.print()
                console.print("[bold cyan]  Top 10 ações do ambiente:[/bold cyan]")
    ...
    # ── history ────────────────────────────────────────────────────────────────
        ...
        try:
            ...
            if not snaps:
                ...
                t.add_column("Risco Médio", width=10)
                ...
                if comp:
                    ...
                    console.print(
                        f"\n  Último vs anterior: risco médio [{dc}]{ds}{abs(comp.avg_risk_delta):.1f}[/{dc}]  "
                        f"{comp.summary}"
                    )
        except Exception as exc:
