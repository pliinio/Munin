...
class AnomalyResult:
    ...
    def to_finding(self) -> Dict:
        ...
        return {
            ...
            "description": (
                "The machine learning anomaly detector identified unusual behaviour "
                "on this host that does not match the baseline of normal hosts. "
                "This may indicate a zero-day attack, misconfiguration, or lateral movement."
            ),
            "severity":    self.anomaly_severity,
            "risk_bonus":  self.anomaly_bonus,
            "remediation": [
                "Investigate the host manually — anomaly may indicate a novel attack vector",
                "Compare current state against a known-good configuration baseline",
                "Check for unexpected processes, connections, and scheduled tasks",
                "Review recently installed software and configuration changes",
            ],
            "detail": self.detail,
    ...
    def detect(self, host_data: Dict) -> Optional[AnomalyResult]:
        ...
        if not is_anomaly:
            return AnomalyResult(
                ...
                anomaly_bonus=0,
                top_features=[],
                detail="No anomaly detected by ML model.",
            )
        ...
        detail = (
            f"ML anomaly score: {anomaly_score:.2f} [{severity}]. "
            f"Key indicators: {', '.join(top_features) if top_features else 'unusual combination of features'}. "
            f"Model trained on {self._n_trained} host(s)."
        )
