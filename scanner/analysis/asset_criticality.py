...
def _classify_by_hostname(hostname: str) -> Tuple[Optional[str], Optional[str], str]:
    ...
    for pattern, asset_type, crit in _HOSTNAME_HINTS:
        if re.search(pattern, hostname, re.IGNORECASE):
            reason = f"Hostname matches pattern '{pattern}'"
...
def classify_asset(host: dict) -> AssetProfile:
    ...
    crit_reason = f"Asset type '{best_name}' defaults to {sig_crit}"

    # Hostname env hint (dev/test/lab → downgrade to LOW)
    if hn_crit == "LOW" and final_crit in ("MEDIUM", "LOW"):
        final_crit  = "LOW"
        crit_reason = f"Hostname suggests non-production environment"
        ...
        if crit_order.get(hn_crit, 0) > crit_order.get(final_crit, 0):
            final_crit  = hn_crit
            crit_reason = f"{hn_reason} → elevated to {hn_crit}"
    ...
    type_reason = (
        f"Matched {len(best_ports)} port(s) and {len(best_svcs)} service(s) "
        f"for type '{best_name}' (score {best_score})"
        if best_score > 0
        else f"Classified as '{best_name}' by heuristic"
    )
