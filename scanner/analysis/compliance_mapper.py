...
_MAPPING_DB: Dict[str, Dict] = {
    ...
    "large_attack_surface": {
        ...
        "control_family": "Attack Surface Management",
        "severity_weight": 2,
    },
    ...
    "cleartext_protocol": {
        ...
        "control_family": "Cryptography & Data Protection",
        "severity_weight": 2,
    },
    ...
    "auth_failure_spike": {
        ...
        "lgpd":         ["Art. 46", "Art. 47"],
        "gdpr":         ["Art. 32"],
        "control_family": "Identity & Authentication",
        "severity_weight": 2,
    },
    ...
    "docker_api_exposed": {
        ...
        "gdpr":         ["Art. 32"],
        "control_family": "Container Security",
        "severity_weight": 4,
    },
    ...
    "ml_anomaly_detected": {
        ...
        "control_family": "Threat Detection",
        "severity_weight": 3,
    },
}
...
class ComplianceScore:
    ...
    overall_label: str = "Good"   # Good | Fair | Poor | Critical
...
def environment_compliance_score(all_hosts: List[Dict]) -> Dict:
    ...
    if avg >= 80:   overall_label = "Good"
    elif avg >= 60: overall_label = "Fair"
    elif avg >= 35: overall_label = "Poor"
    else:           overall_label = "Critical"
...
def format_compliance_report(cs: ComplianceScore, host_ip: str = "") -> str:
    ...
    """
    lines = []
    header = f"Compliance Posture — {host_ip}" if host_ip else "Compliance Posture"
    lines.append("=" * 60)
    lines.append(header)
    lines.append("=" * 60)
    lines.append("")
    lines.append(f"  Overall:          {cs.overall_label}")
    ...
    lines.append(f"  LGPD Exposure:    {cs.lgpd_exposure}")
    lines.append("")

    if cs.nist_by_function:
        lines.append("  NIST CSF by Function:")
        for func, func_name in _NIST_FUNCTIONS.items():
            score = cs.nist_by_function.get(func, 100.0)
            bar   = "█" * int(score / 10) + "░" * (10 - int(score / 10))
            lines.append(f"    {func} {func_name:<10}  {bar}  {score:.0f}%")
        lines.append("")

    if cs.failed_families:
        lines.append(f"  Control Families with Failures ({len(cs.failed_families)}):")
        for fam in cs.failed_families:
            lines.append(f"    - {fam}")
        lines.append("")

    if cs.failed_controls:
        lines.append(f"  Failed Controls (sample):")
        for ctrl in cs.failed_controls[:8]:
            lines.append(f"    - {ctrl}")
        if len(cs.failed_controls) > 8:
            lines.append(f"    ... and {len(cs.failed_controls) - 8} more")
