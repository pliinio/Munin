...
def correlate(
    ...
    if 22 in open_port_nums and metrics["failed_ssh_logins"] >= SSH_BRUTEFORCE_THRESHOLD:
        findings.append(_finding(
            "ssh_brute_force",
            f"{metrics['failed_ssh_logins']} tentativa(s) de login SSH malsucedida(s) nos logs",
        ))
        ...
        for cve in p.get("cves", []):
            ...
            if float(cvss) >= HIGH_CVE_CVSS:
                ...
                high_cve_ports.append(
                    f"porta {p['port']} ({svc}) — {cve['id']} CVSS {cvss}"
                )
                break
    ...
    if web_errors >= WEB_ERROR_THRESHOLD and web_with_cve:
        findings.append(_finding(
            "web_error_with_cve",
            f"{web_errors} erro(s) HTTP nos logs + CVE(s) no serviço web",
        ))
    ...
    if exposed_critical:
        ...
        findings.append(_finding(
            "critical_port_open",
            f"Porta(s) crítica(s) aberta(s): {port_list}",
        ))
    ...
    if len(open_port_nums) >= LARGE_SURFACE_THRESHOLD:
        findings.append(_finding(
            ...
            f"{len(open_port_nums)} portas abertas detectadas",
        ))
        ...
        for script_name, output in scripts.items():
            if "VULNERABLE" in str(output).upper():
                confirmed_vuln_scripts.append(
                    f"{script_name} na porta {port_num}"
                )
    ...
    if exposed_cleartext:
        ...
        findings.append(_finding(
            "cleartext_protocol",
            f"Porta(s) de protocolo em texto claro aberta(s): {port_list}",
        ))
    ...
    if exposed_db:
        ...
        findings.append(_finding(
            "database_exposed",
            f"Porta(s) de banco de dados acessível(is): {port_list}",
        ))
    ...
    if not already_ssh and metrics["auth_failures"] >= AUTH_FAILURE_THRESHOLD:
        findings.append(_finding(
            "auth_failure_spike",
            f"{metrics['auth_failures']} falha(s) de autenticação nos logs",
        ))
    ...
    if exposed_docker:
        ...
        findings.append(_finding(
            "docker_api_exposed",
            f"API do daemon Docker acessível na(s) porta(s): {port_list}",
        ))
    ...
    if ml_finding:
        ...
        if "ml_anomaly_detected" not in rule_ids:
            ...
            if "CRITICAL" in rule_severities:
                ...
                ml_finding["name"]       = "Anomalia ML (consultiva — regras já críticas)"
