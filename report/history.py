...
def compare_snapshots(old: Snapshot, new: Snapshot) -> ComparisonReport:
    ...
    if avg_delta < -5:    summary = f"Risco melhorou: pontuação média caiu {abs(avg_delta):.1f} pts desde o último scan."
    elif avg_delta > 5:   summary = f"Risco aumentou: pontuação média subiu {avg_delta:.1f} pts desde o último scan."
    else:                 summary = "Postura de risco estável desde o scan anterior."

    if worsened > 0:
        summary += f" {worsened} host(s) deterioraram."
    if improved > 0:
        summary += f" {improved} host(s) melhoraram."
