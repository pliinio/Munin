...
def print_summary(result: dict) -> None:
    ...
    t.add_row("Alvo",              meta.get("target", "N/A"))
    t.add_row("Início do scan",    meta.get("scan_time", "N/A"))
    t.add_row("Duração",           f"{meta.get('scan_duration', 0):.1f} s")
    t.add_row("Hosts encontrados", str(len(hosts)))
    t.add_row("Portas abertas",    str(total_ports))
    t.add_row("CVEs encontradas",  str(total_cves))
    ...
    console.print(
        Panel(t, title="[bold cyan]Resumo do Scan[/bold cyan]",
...
def print_host(host: dict) -> None:
    ...
    info.add_row("Fabricante", host.get("mac_vendor", "Desconhecido"))
    info.add_row("Hostname", host.get("hostname", "N/A"))

    os_name = os_info.get("name", "Desconhecido")
    acc     = os_info.get("accuracy", 0)
    os_str  = f"{os_name}  [dim]({acc}% de confiança)[/dim]" if acc else os_name
    info.add_row("SO",       os_str)
    info.add_row("Tipo de SO", os_info.get("type", "Desconhecido"))
    ...
    if ports:
        ...
        pt.add_column("Porta",   width=7,  no_wrap=True)
        pt.add_column("Proto",   width=6,  no_wrap=True)
        pt.add_column("Estado",  width=9,  no_wrap=True)
        pt.add_column("Serviço", width=12, no_wrap=True)
        pt.add_column("Produto / Versão", width=36)
    ...
    for port_num, scripts in host.get("vulnerabilities", {}).items():
        for script_name, output in scripts.items():
            ...
            if "VULNERABLE" in out_str.upper() or "vuln" in script_name.lower():
                console.print(
                    f"  [bold red]VULNERÁVEL  {script_name}  (porta {port_num})[/bold red]"
                )
...
def print_findings(findings: list, score: int, level: str) -> None:
    ...
    t.add_column("Severidade", width=10, no_wrap=True)
    t.add_column("Ameaça",     width=30, no_wrap=True)
    t.add_column("Detalhe",    width=60)
    ...
    console.print(
        Rule(
            f"[bold yellow]Análise de Ameaças  [{color}]{level} {score}/100[/{color}][/bold yellow]",
            style="yellow",
        )
    )
    ...
    if steps:
        console.print("  [bold cyan]Ações Recomendadas[/bold cyan]")
    ...
    if explanations:
        console.print("  [bold cyan]Por que estas ameaças foram sinalizadas[/bold cyan]")
...
def print_all(result: dict) -> None:
    ...
    console.rule(f"[bold cyan]Detalhes dos Hosts  ({len(hosts)} hosts encontrados)[/bold cyan]")
    console.print()
    ...
    if critical_hosts:
        ...
        console.rule("[bold red]Ameaças Prioritárias[/bold red]")
        console.print()
